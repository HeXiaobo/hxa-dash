...
| `HXA_AUTH_SECRET` | Strong random HMAC secret. Rotating it logs everyone out. |
...
| `HXA_INGEST_API_KEY` | Set to the same value as `HEALTH_API_KEY` for the first rollout. |
...
| `POST /api/report/activity` | activity reporter | Deploy the updated reporter or pass `--api-key`; it reads `HXA_INGEST_API_KEY` or `HEALTH_API_KEY`. |
...
3. Confirm `HXA_AUTH_ENABLED=false` is present in PM2 env.
...
7. Confirm activity reporters are updated or launched with `--api-key`.
8. Post deploy commit, rollback commit, and this checklist result on #4.
...
5. Verify machine ingest with a key:
   ...
   curl -fsS -X POST https://hxa.zhiw.ai/api/report \
     ...
     -d '{"name":"deploy-smoke","status":"smoke"}'
   ```
...
7. Smoke test the auth boundary:
   ...
   curl -fsS -X POST https://hxa.zhiw.ai/api/report \
     -H "Content-Type: application/json" \
     -H "X-API-Key: $HXA_INGEST_API_KEY" \
     -d '{"name":"deploy-smoke","status":"auth-smoke"}'
   ```
